...
export default class Game6 extends Core {
    constructor() {
        ...
        this.countLvlTrue = 0;
        this.dragZone = [];
        this.taskTitle = "Визнач до якого зображення підходить кожен з нижченаведених описів";
    }
    ...
    makeAnswersList() {
        ...
        for(let i = 0; i < this.answers.length; i++) {
            this.manifest.push(
                {src:`../images/${this.answers[i].img}`, id:`img-${i}`}
            );
        ...
        this.answers2 = answers.sort(() => (Math.random() > 0.5) ? 1 : -1);
    ...
    handleLvlImageLoad() {
        for(let i = 0; i < this.images.length; i++) {
            ...
            let imageBounds = new createjs.Rectangle(0, 0, 200, 150)
    ...
    eventPressUp(i, event) {
        ...
        for(let j = 0; j < this.dragZone.length; j++) {
            ...
            if (collision && !this.dragZone[j].notEmpty) {
                ...
                let thisValue = this.answers[i].id;
                console.log(dropZoneValue, thisValue);
